...
lint_eol_spaces() {
    ...
    # Force grep to return true because of pipefail
    # We use the count as a metric, not the exit code
    spacelines="$(grep -Pc '\s+$' "$1" || true)"
...
lint_shell() {
    ...
    for script in $shell_scripts; do
        ...
        # Shellcheck
        if [[ ! "$should_skip_shellcheck" ]]; then
            shellcheck "$script"
...
lint_other_scripts() {
    source_files=$(find . ! -regex '.+\.\(c\|h\|exp\|sh\|S\|s\|bin\|ld\|out\|log\|asm\|img\|swp\)' \
                            ! -name 'tags' \
                            ! -name 'bootblock' \
                            ! -name 'dot-bochsrc' \
                            ! -path './docs/*' \
...
if [[ ! "$SKIP_LINT" = "true" ]]; then
    ...
    lint_expect_scripts
    echo "[    LINT] Expect scripts OK"

    echo "[    LINT] Other scripts"
    lint_other_scripts
    echo "[    LINT] Other scripts OK"
