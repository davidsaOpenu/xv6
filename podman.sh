...
SKIP_LINT=false
SKIP_BUILD=false
SKIP_GUEST_TESTS=true # Runs through SSH
SKIP_HOST_TESTS=true  # Runs through SSH
...
lint_clang_format() {
    ...
    for f in $source_files; do
        ...
        clang-format -n --Werror "$f" 2>&1 1>/dev/null | sed "s/^/[CLANG-FORMAT]\t/";

        # shellcheck disable=SC2181 # Because of sed
        if [ "$?" -ne 0 ]; then
            echo "FAILED ON CLANG-FORMAT: $f"
            exit 1
        fi
...
if [[ ! "$SKIP_GUEST_TESTS" = "true" ]]; then
    ./tests/run-guest-tests.exp 2>&1 | sed 's/^/[  EXPECT]\t/'
else
