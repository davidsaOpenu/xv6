...
enum event_type { ADD_EVENT, REWRITE_EVENT, DELETE_EVENT };
...
static void finish_rewrite_event() {
  ...
  if (logbook.rewrite_event.new_object_table_index == -1) {
    ...
    err =
        get_objects_table_index(logbook.rewrite_event.new_object_id,
    ...
    memmove_into_vector_bytes(templogbookvector, 0, (char*)&logbook,
                              sizeof(logbook));
    cache_delete_object(LOGBOOK_OBJECT_ID);
    err = rewrite_object(templogbookvector, sizeof(logbook), 0, LOGBOOK_OBJECT_ID);
    if (err != NO_ERR) {
      releasesleep(&loglock);
  ...
  cache_remove_object(logbook.rewrite_event.old_object_id, 0, true);
}
...
static void finish_log_transactions() {
  ...
  if (err != NO_ERR) {
    releasesleep(&loglock);
    panic(
        "logbook - unexpected error when loading the logbook event object "
        "size");
  }
  ...
  memmove_from_vector((char*)&logbook, logbookvector, 0,
                      logbookvector.vectorsize);
  switch (logbook.type) {
    case ADD_EVENT:
      finish_add_event();
...
uint log_add_object(const void* object, uint size, const char* name) {
  ...
  if (add_object(&logbook, sizeof(logbook), LOGBOOK_OBJECT_ID) !=
      NO_ERR) {
    releasesleep(&loglock);
...
uint log_rewrite_object(const void* object, uint size, const char* name) {
  ...
  err = get_objects_table_index(name,
...
uint log_get_object(const char* name, vector* outputvector,
  ...
  acquiresleep(&loglock);
  uint result = cache_get_object(name, outputvector, read_object_from_offset, OBJ_END);
  releasesleep(&loglock);
  return result;
}
