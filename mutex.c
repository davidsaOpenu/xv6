...
bool g__mutex_initialized = false;

/**
 * This method initializes the mutex directory, if it doesn't exist,
 * on first call - to enable the mutex to work.
 */
static enum mutex_e initalize_mutex() {
  if (g__mutex_initialized) return MUTEX_SUCCESS;
  g__mutex_initialized = true;
...
enum mutex_e mutex_init_named(mutex_t *const mutex_var,
  ...
  enum mutex_e res;
  if ((res = initalize_mutex()) != MUTEX_SUCCESS) return res;
