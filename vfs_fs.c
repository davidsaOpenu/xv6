...
static struct vfs_inode *vfs_namex(char *path, int nameiparent, char *name,
  ...
  while ((path = vfs_skipelem(path, name)) != 0) {
    ...
    mntinum = ip->inum;
    ip->i_op.iunlockput(ip);
    if ((!vfs_namencmp(name, "..", 3)) && curmount != 0 && (curmount->dev != ROOTDEV) &&
        ((mntinum == ROOTINO) || (mntinum == OBJ_ROOTINO)) && curmount->mountpoint != 0 &&
...
int vfs_namencmp(const char *s, const char *t, int length) { return strncmp(s, t, length); }
