...
// Function to parse the input buffer
// return the the command before the new line character
char *parse_input_buffer() {
  // We need to extract the command from the input buffer because
  // it may contain some characters that are not part of the command
  // or more than one command. We'll use the read and write pointers

  // Find the start of the current command
  int start = input.r;
  while (start < input.e && input.buf[start] == ' ') {
    start++;
  }

  // Find the end of the current command
  int end = start;
  while (end < input.e && input.buf[end] != '\n' && input.buf[end] != '\0') {
    end++;
  }
  ...
  // now we need to copy the command to a new buffer
