 ...
 * number
 */
void generate_file_name_for_number(int n, char *buffer, char *basename) {
  int cc = 0;
  while (basename[cc] != 0) {
    buffer[cc] = basename[cc];
    cc++;
  }
...
void objcachestressbigworkingset() {
  ...
  for (int i = 0; i < 2 * cache_entries; i++) {
    generate_file_name_for_number(i, buf, "bigwset");
...
void objcachestresssmallworkingset() {
  ...
  int cache_entries = object_cache_number_of_blocks();
  for (int i = 0; i < cache_entries / 2; i++) {
    generate_file_name_for_number(i, buf, "smallwset");
  ...
  for (int c = 0; c < 5; c++) {
    for (int i = 0; i < cache_entries / 2; i++) {
      ...
      if (fd < 0) {
        printf(1,
               "objcachestresssmallworkingset: could not open file for "
               "reading%s\n",
               buf);
        exit(1);
      }
