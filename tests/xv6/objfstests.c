 ...
 * we get the expected outputs
 */
void objcachestress() {
  printf(1, "objcachestress starting\n");
  int fd, cc;
  ...
  for (int i = 0; i < 2 * cache_entries; i++) {
    ...
    if (fd < 0) {
      printf(1, "could not open file for reading%s\n", buf);
      exit(1);
    }
    char *buf2 = buf + 1000;
    ...
    if (strcmp(buf, buf2) != 0) {
      printf(1,
             "contents of file %s are not as expected: the expected is %s but "
             "we got %s\n",
             buf, buf, buf2);
      exit(1);
    }
  ...
  printf(1, "objcachestress finished successfully\n");
}
