...
/* Verify init
  ...
  - make sure no tty devices in use, e.g. make sure pouch containers occupy no
  tty device.
...
/* Verify ioctl syscall connect/disconnect
   ...
   using TTY_SET/DEV_CONNECT : TTY_GET/DEV_CONNECT will return "1" and
   TTY_GET/DEV_DISCONNECT will return "0". TTYSETS return "0" on operation
   success.
*/
...
int ioctl_console_test() {
  ...
  if (ioctl(tty_fd, TTYGETS, DEV_CONNECT) == 0) {
    printf(stderr,
           " %s  connected TTYGETS / DEV_CONNECT, expect: disconnected \n",
           tty_name);
    return -1;
  }
...
/* Verify wrong device usage
  ...
  - mknod()is xv6 syscall to create a device, user should specify minor and
  major numbers
*/
...
int main() {
  // TTY INIT TESTS
  if (init_tests() < 0) exit(1);

  // ioctl TESTS
  if (ioctl_wrong_device_test() < 0) exit(1);

  if (ioctl_console_test() < 0) exit(1);
