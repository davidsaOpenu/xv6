...
void set_cpu_weight_output_string(char* buf, int number) {
  ...
  int num_str_length = 0;

  // prepare cpu weight output string
  strcat(buf, number);
  num_str_length = utoa(cpu_weight_number, number);
  strncat(buf, cpu_weight_number, num_str_length);
  strncat(buf, "\n", strlen("\n"));
}
...
TEST(test_cpu_weight) {
  ...
  ASSERT_TRUE(child_cpu_percentage < 40 && child_cpu_percentage > 10);
