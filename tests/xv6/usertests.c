...
#define NATIVEFS_CACHE_TEST_FACTOR (12)
...
void bigargtest(void) {
  ...
  if (pid == 0) {
    ...
    int i;
    for (i = 0; i < MAXARG - 1; i++) args[i] = long_arg;
    args[MAXARG - 1] = 0;
...
void set_fs_cache_state(int enable) {
  int fd = -1;
  char *state;
  ...
  if (0 != strncmp(proc_cache_state, state, 2)) {
    printf(stdout, "Failed to set fs cache to %d\n", enable);
    exit(1);
  }
