...
TEST(super_block_object) {
  ...
  ASSERT_NO_ERR(get_object(TESTED_DEVICE, SUPER_BLOCK_ID, &sb, 0));
...
TEST(table_object) {
  ...
  ASSERT_NO_ERR(get_object(TESTED_DEVICE, OBJECT_TABLE_ID, &table, 0));
...
TEST(add_single_object) {
  ...
  ASSERT_NO_ERR(add_object(TESTED_DEVICE, my_string, strlen(my_string) + 1,
                           "simple_string"));
  uint size;
  ...
  ASSERT_NO_ERR(get_object(TESTED_DEVICE, "simple_string", actual, 0));
  ASSERT_UINT_EQ(0, strcmp(actual, my_string));
}

TEST(add_object_already_exist) {
  uint placeholder = 0;
  ASSERT_NO_ERR(add_object(TESTED_DEVICE, &placeholder, sizeof(placeholder),
                           "already_exist_0"));
  ASSERT_UINT_EQ(OBJECT_EXISTS,
                 add_object(TESTED_DEVICE, &placeholder, sizeof(placeholder),
                            "already_exist_0"));
}
...
TEST(delete_existing_object) {
  uint placeholder = 0;
  ASSERT_NO_ERR(
      add_object(TESTED_DEVICE, &placeholder, sizeof(placeholder), "delete_0"));
  uint size;
...
TEST(rewrite_existing_object_with_shorter_data) {
  ...
  ASSERT_NO_ERR(add_object(TESTED_DEVICE, first_string,
                           strlen(first_string) + 1, "rewrite_shorter"));
  ...
  ASSERT_NO_ERR(rewrite_object(TESTED_DEVICE, second_string_vector,
                               sizeof(second_string), 0, "rewrite_shorter"));
  ...
  //  ASSERT_TRUE(actual != NULL);
  //  ASSERT_NO_ERR(get_object(TESTED_DEVICE, "rewrite_shorter", actual, 0));
...
TEST(rewrite_existing_object_with_longer_data) {
  ...
  ASSERT_NO_ERR(add_object(TESTED_DEVICE, first_string,
                           strlen(first_string) + 1, "rewrite_longer"));
  ...
  memmove_into_vector_bytes(second_string_vector, 0, second_string,
                            sizeof(second_string));
  ASSERT_NO_ERR(rewrite_object(TESTED_DEVICE, second_string_vector,
                               sizeof(second_string), 0, "rewrite_longer"));
  ...
  char* actual = (char*)malloc(strlen(second_string) + 1);
  ASSERT_TRUE(actual != NULL);
  ASSERT_NO_ERR(get_object(TESTED_DEVICE, "rewrite_longer", actual, 0));
  ASSERT_UINT_EQ(0, strcmp(actual, second_string));
  free(actual);
}
...
TEST(writing_multiple_objects) {
  ...
  for (uint i = 0; i < 3; ++i) {
    ASSERT_NO_ERR(add_object(TESTED_DEVICE, objects_data[i],
                             strlen(objects_data[i]) + 1, objects_name[i]));
  }
  for (uint i = 0; i < 3; ++i) {
    uint size;
    ...
    ASSERT_NO_ERR(get_object(TESTED_DEVICE, objects_name[i], actual_data, 0));
...
TEST(add_to_full_table) {
  ...
  for (uint i = 0; i < num_of_free_entries; ++i) {
    ...
    ASSERT_NO_ERR(
        add_object(TESTED_DEVICE, object_data_ptr, object_size, object_id));
  }

  vector last_obj_vector = newvector(sizeof(object_size), 1);
  memmove_into_vector_bytes(last_obj_vector, 0, (void*)object_data_ptr,
                            object_size);
  ASSERT_NO_ERR(rewrite_object(TESTED_DEVICE, last_obj_vector, object_size, 0,
                               object_id));

  ASSERT_UINT_EQ(OBJECTS_TABLE_FULL,
                 add_object(TESTED_DEVICE, &object_data, sizeof(object_data),
                            "non existing object"));
}

uint find_object_offset(const char* object_name) {
  ObjectsTableEntry table[INITIAL_OBJECT_TABLE_SIZE];
  get_object(TESTED_DEVICE, OBJECT_TABLE_ID, &table, 0);
  uint address = -1;
...
TEST(reusing_freed_space) {
  uint data = 0;
  ASSERT_NO_ERR(
      add_object(TESTED_DEVICE, &data, sizeof(data), "reusing object 1"));
  ...
  ASSERT_NO_ERR(
      add_object(TESTED_DEVICE, &data, sizeof(data), "reusing object 2"));
...
TEST(add_when_there_is_no_more_disk_left) {
  ASSERT_UINT_EQ(NO_DISK_SPACE_FOUND,
                 add_object(TESTED_DEVICE, NULL, STORAGE_DEVICE_SIZE + 1,
                            "object too large"));
}
