...
TEST(rewrite_existing_object_with_shorter_data) {
  ...
  ASSERT_NO_ERR(rewrite_object(second_string_vector, sizeof(second_string),
                               0, "rewrite_shorter"));
...
TEST(add_to_full_table) {
  ...
  memmove_into_vector_bytes(last_obj_vector, 0,
                            (void*)object_data_ptr, object_size);
  ASSERT_NO_ERR(rewrite_object(last_obj_vector, object_size, 0, object_id));

  ASSERT_UINT_EQ(OBJECTS_TABLE_FULL,
                 add_object(&object_data, sizeof(object_data),
                            "non existing object"));
}
