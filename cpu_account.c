...
void cpu_account_schedule_proc_update(struct cpu_account* cpu, struct proc* p) {
  ...
  if (cpu->cpu_account_frame > p->cpu_account_frame) {
    p->cpu_percent = p->cpu_period_time * 100 / (cpu->cpu_account_period*ncpu);
...
int cpu_account_schedule_process_decision_by_weights(struct cpu_account * cpu, struct proc * p) {
    ...
    while (cgroup != 0) { // loop until the very first genaration
        ...
        expected_cpu_percent =  expected_cpu_percent * my_weight / total_generation_weight;
    ...
    int rc = 0;
    // the process's percent of cpu time from the total cpus time of the current frame.
    int real_cpu_percent = p->cpu_period_time * 100 / curr_frame_toal_process_and_cpu_time;
