...
int unsafe_cg_read(cg_file_type type, struct vfs_file* f, char* addr, int n) {
  if (type == CG_FILE) {
    ...
    if (f->readable == 0 || *f->cgp->cgroup_dir_path == 0) return -1;
    ...
    if (filename_const == CGROUP_PROCS) {
      ...
      r = copy_buffer_up_to_end(eventstext + f->off, min(sizeof(eventstext), n),
                                addr);

    } else if (filename_const == CGROUP_MAX_DESCENDANTS) {
      itoa(buf, f->cgp->max_descendants_value);
      copy_buffer_up_to_end_replace_end_with_newline(buf, min(sizeof(buf), n),
                                                     addr);
      ...
      copy_and_move_buffer(&stattextp, "usage_usec - ",
                           strlen("usage_usec - "));
      ...
      copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
      copy_and_move_buffer(&stattextp, "system_usec - ",
                           strlen("system_usec - "));
      copy_and_move_buffer(&stattextp, system_buf, strlen(system_buf));

      if (f->cpu.stat.active) {
        copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
        copy_and_move_buffer(&stattextp, "nr_periods - ",
                             strlen("nr_periods - "));
        copy_and_move_buffer(&stattextp, nr_periods_buf,
                             strlen(nr_periods_buf));
        copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
      ...
      r = copy_buffer_up_to_end(weighttext + f->off,
      ...
      r = copy_buffer_up_to_end(maxtext + f->off,
      ...
      copy_and_move_buffer(&stattextp, "num_of_procs - ",
                           strlen("num_of_procs - "));
      copy_and_move_buffer(&stattextp, nr_of_procs_buf,
                           strlen(nr_of_procs_buf));
      copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
      ...
      r = copy_buffer_up_to_end(cputext + f->off,
      ...
      r = copy_buffer_up_to_end(frztext + f->off,
      ...
      uint stattext_size =
          ...
          utoa(file_dirty_aggregated_buf, f->mem.stat.file_dirty_aggregated) +
          1 + strlen("pgfault - ") + utoa(pgfault_buf, f->mem.stat.pgfault) +
          1 + strlen("pgmajfault - ") +
      ...
      copy_and_move_buffer(&stattextp, "file_dirty - ",
                           strlen("file_dirty - "));
      ...
      copy_and_move_buffer(&stattextp, "pgmajfault - ",
                           strlen("pgmajfault - "));
