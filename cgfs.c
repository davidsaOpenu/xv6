 ...
 * terminator character in the input buffer. Receives char parameter pointer
 * "src", int parameter "size", char parameter pointer "dest".
 *
 ...
 * The function returns the amount of bytes copied to the dest buffer.
 */
static inline int copy_buffer_up_to_end_replace_end_with_newline(char* src,
                                                                 int size,
                                                                 char* dest) {
  int i;
  for (i = 0; i < size; i++) {
 ...
 *
 * The function copies from "string" into "buffer" at most "size" amount of
 * characters. In addition "buffer" index is advanced by "size".
 */
...
static inline void copy_and_move_buffer_max_len(char** buffer, char* string) {
  // Cpplint requires #include <string> which is not suitable here.
  strncpy(*buffer, string,
          strlen(string));  // NOLINT(build/include_what_you_use)
  *buffer += MAX_CGROUP_FILE_NAME_LENGTH;
}

/**
 * This function copies from a give source into a given destination based on the
 * input parameters.
 *
 * Receives char parameter pointer "destination", char parameter pointer
 * "source", int parameter "var".
 *
 * The function copies from "source" into "destination" "var" amount of
 * characters. In addition "var" index is advanced by length of string "source".
 */
...
static int find_procs_offsets(int* procoff, int* pidoff, struct vfs_file* f) {
  ...
  *pidoff = f->off;
  ...
  if (*procoff >= sizeof(f->cgp->proc) / sizeof(*f->cgp->proc)) return -1;
...
int is_cg_file_statable(struct vfs_file* f) {
...
static int unsafe_cg_open_dir(char* dirname, struct cgroup* cgp, int omode) {
  ...
  if (*cgp->cgroup_dir_path == 0) return -1;
...
static int read_file_cg_procs(struct vfs_file* f, char* addr, int n) {
  int procoff;
  int pidoff;
  int r = 0;
...
static int read_file_cg_controllers(struct vfs_file* f, char* addr, int n) {
  int i = 0;
...
static int read_file_cg_subtree(struct vfs_file* f, char* addr, int n) {
  int i = 0;
  memset(buf, '\0', MAX_STR);
...
static int read_file_cg_stat(struct vfs_file* f, char* addr, int n) {
  ...
  copy_and_move_buffer(&stattextp, "nr_descendants - ",
                       strlen("nr_descendants - "));
  copy_and_move_buffer(&stattextp, nr_descendants_buf,
                       strlen(nr_descendants_buf));
  copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
  copy_and_move_buffer(&stattextp, "nr_dying_descendants - ",
                       strlen("nr_dying_descendants - "));
  copy_and_move_buffer(&stattextp, nr_dying_descendants_buf,
                       strlen(nr_dying_descendants_buf));
  copy_and_move_buffer(&stattextp, "\n", strlen("\n"));

  return copy_buffer_up_to_end(
      stattext + f->off, min(at_least_zero(stattextp - stattext - f->off), n), addr);
}
...
static int read_file_cpu_stat(struct vfs_file* f, char* addr, int n) {
  ...
  if (f->cpu.stat.active) {
    ...
    copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
    copy_and_move_buffer(&stattextp, "nr_throttled - ",
                         strlen("nr_throttled - "));
    copy_and_move_buffer(&stattextp, nr_throttled_buf,
                         strlen(nr_throttled_buf));
    copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
    copy_and_move_buffer(&stattextp, "throttled_usec - ",
                         strlen("throttled_usec - "));
    copy_and_move_buffer(&stattextp, throttled_usec_buf,
                         strlen(throttled_usec_buf));
  }
...
static int read_file_cpu_weight(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(weighttext + f->off,
                               min(at_least_zero(weighttextp - weighttext - f->off), n),
                               addr);
}
...
static int read_file_cpu_max(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(maxtext + f->off,
                               min(at_least_zero(maxtextp - maxtext - f->off), n), addr);
}
...
static int read_file_set_cpu(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(cputext + f->off,
                               min(at_least_zero(cputextp - cputext - f->off), n), addr);
}
...
static int read_file_set_frz(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(frztext + f->off,
                               min(at_least_zero(frztextp - frztext - f->off), n), addr);
}
...
static int read_file_mem_stat(struct vfs_file* f, char* addr, int n) {
  ...
  uint stattext_size =
      strlen("file_dirty - ") + utoa(file_dirty_buf, f->mem.stat.file_dirty) +
      1 + strlen("file_dirty_aggregated - ") +
      utoa(file_dirty_aggregated_buf, f->mem.stat.file_dirty_aggregated) + 1 +
      strlen("pgfault - ") + utoa(pgfault_buf, f->mem.stat.pgfault) + 1 +
      strlen("pgmajfault - ") + utoa(pgmajfault_buf, f->mem.stat.pgmajfault) +
      2 + strlen("kernel - ") + utoa(kernel_buf, f->mem.stat.kernel);
  ...
  copy_and_move_buffer(&stattextp, "file_dirty_aggregated - ",
                       strlen("file_dirty_aggregated - "));
  copy_and_move_buffer(&stattextp, file_dirty_aggregated_buf,
                       strlen(file_dirty_aggregated_buf));
  copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
...
static int read_file_cg_events(struct vfs_file* f, char* addr, int n) {
  ...
  if (f->cgp->populated) eventstext[strlen("popluated - ")] = '1';

  if (f->cgp->is_frozen) eventstext[strlen(eventstext) - 2] = '1';

  return copy_buffer_up_to_end(eventstext + f->off, min(sizeof(eventstext), n),
                               addr);
}
...
int unsafe_cg_read(cg_file_type type, struct vfs_file* f, char* addr, int n) {
  ...
  if (type == CG_FILE) {
    ...
    if (*f->cgp->cgroup_dir_path == 0) return -1;
...
static int write_file_cg_subtree(struct vfs_file* f, char* addr, int n) {
  ...
  if (cpucontroller == 1 &&
      unsafe_enable_cpu_controller(f->cgp) <= RESULT_ERROR)
    return -1;
  if (cpucontroller == 2 &&
      unsafe_disable_cpu_controller(f->cgp) <= RESULT_ERROR)
    return -1;

  if (pidcontroller == 1 &&
      unsafe_enable_pid_controller(f->cgp) <= RESULT_ERROR)
    return -1;
  if (pidcontroller == 2 &&
      unsafe_disable_pid_controller(f->cgp) <= RESULT_ERROR)
    return -1;

  if (setcontroller == 1 &&
      unsafe_enable_set_controller(f->cgp) <= RESULT_ERROR)
    return -1;
  if (setcontroller == 2 &&
      unsafe_disable_set_controller(f->cgp) <= RESULT_ERROR)
    return -1;

  if (memcontroller == 1 &&
      unsafe_enable_mem_controller(f->cgp) <= RESULT_ERROR)
    return -1;
  if (memcontroller == 2 &&
...
static int write_file_cg_max_descen(struct vfs_file* f, char* addr, int n) {
  if (atoi(addr) < 0 || strlen(addr) > 2) return -1;
  f->cgp->max_descendants_value = atoi(addr);
  return n;
}

static int write_file_max_depth(struct vfs_file* f, char* addr, int n) {
  if (atoi(addr) < 0 || strlen(addr) > 2) return -1;
  f->cgp->max_depth_value = atoi(addr);
  return n;
}

static int write_file_cpu_max(struct vfs_file* f, char* addr, int n) {
  ...
  // sh.c doesn't treat space inside for example: "1000 20000" as a single
  // argument so we will use special format, this also allows to parse zeroes
  // inside a value

  while (*addr != ',' && *addr != '\0' && i < (sizeof(max_string) - 1)) {
    max_string[i] = *addr;
    i++;
    addr++;
  }
...
static int write_file_pid_max(struct vfs_file* f, char* addr, int n) {
  char max_string[32] = {0};
  int max = -1;
  int i = 0;
...
static int write_file_set_cpu(struct vfs_file* f, char* addr, int n) {
  char set_string[32] = {0};
  int set = -1;
  int i = 0;

  while (*addr != ',' && *addr != '\0' && i < (sizeof(set_string) - 1)) {
    set_string[i] = *addr;
    i++;
    addr++;
  }
...
static int write_file_set_frz(struct vfs_file* f, char* addr, int n) {
  char set_string[32] = {0};
  int set_freeze = -1;
  int i = 0;
...
static int write_file_mem_max(struct vfs_file* f, char* addr, int n) {
  char max_string[32] = {0};
  unsigned int max = -1;
  int i = 0;
...
static int write_file_mem_min(struct vfs_file* f, char* addr, int n) {
  char min_string[32] = {0};
  unsigned int min = -1;
  int i = 0;

  while (*addr != ',' && *addr != '\0' && i < (sizeof(min_string) - 1)) {
    min_string[i] = *addr;
    i++;
    addr++;
  }
...
int get_base_name(char* path, char* base_name) {
  ...
  char* temp = fpath;
  while (*temp != '\0') {
    if (*temp == '/') file_name_temp = temp;
    temp++;
  }

  if (file_name_temp == fpath || file_name_temp == temp - 1) return -1;

  temp = file_name_temp + 1;
  while (*temp != '\0') *base_name++ = *temp++;
...
int get_dir_name(char* path, char* dir_name) {
  ...
  temp = fpath;
  while (temp < file_name_temp) *dir_name++ = *temp++;
...
static int cg_file_size(struct vfs_file* f) {
  ...
  if (filename_const == CGROUP_PROCS) {
    ...
    if (f->cgp->cpu_controller_avalible) size += 3;
    if (f->cgp->pid_controller_avalible) size += 3;
  } else if (filename_const == CGROUP_SUBTREE_CONTROL) {
    if (f->cgp->cpu_controller_enabled) size += 3;
    if (f->cgp->pid_controller_enabled) size += 3;
    ...
    size += strlen("nr_descendants - ") + intlen(f->cgp->nr_descendants) +
            strlen("\n") + strlen("nr_dying_descendants - ") +
...
int unsafe_cg_stat(struct vfs_file* f, struct stat* st) {
  ...
  if (*f->cgfilename == 0) {
    ...
    if (!is_cg_file_statable(f)) return -1;
    st->type = T_CGFILE;
