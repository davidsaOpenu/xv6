...
static void* find_empty_space(uint size) {
  ...
  if (populated_size == 0 &&
      STORAGE_DEVICE_SIZE - size >= super_block.store_offset) {
  ...
  if (earliest_occupied_entry->disk_offset - super_block.store_offset >= size) {
    ...
    uint needed_space = size - (earliest_occupied_entry->disk_offset -
                                super_block.store_offset);
    uint offset_after_last_entry =
        entry_index_to_entry_offset(last_occupied_entry + 1);
...
uint rewrite_object(vector data, uint objectsize, uint write_starting_offset,
                    const char* name) {
  uint err;
