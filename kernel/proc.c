...
char procfs_root[MAX_PATH_LENGTH] = {0};
...
struct proc *proc_by_pid(int pid) {
  ...
  for (struct proc *proc_iter = ptable.proc; proc_iter < &ptable.proc[NPROC]; ++proc_iter) {
...
found:
  p->state = EMBRYO;
  p->killed = 0;  // Prevents process allocating with killed=1 flag. This
                  // behavior was exhibited as a bug and this prevents it.
  release(&ptable.lock);
...
int growproc(int n) {
  ...
  sz = curproc->sz;
  if (n > 0) {  // In this case we update protected memory inside of allocuvm
                // function
    if ((sz = allocuvm(curproc->pgdir, sz, sz + n, cgroup)) == 0) return -1;
  } else if (n < 0) {
...
int fork(void) {
  ...
  // Check if the current process has a child pid namespace and if its pid1 was
  // killed.
  if (curproc->child_pid_ns && curproc->child_pid_ns->pid1_ns_killed) {
    return -1;
  }

  // In case trying to fork new process and the cgroup reached its process
  // limit, given pid controller is enabled, return failure
  if (curproc->cgroup->pid_controller_enabled &&
      (curproc->cgroup->num_of_procs + 1) > curproc->cgroup->max_num_of_procs)
    return -1;

  // In case trying to fork a new process and the cgroup reached its memory
  // limit, given memory controller is enabled, return failure
  if (curproc->cgroup->mem_controller_enabled &&
...
void kill_all_pid_ns(struct proc *parent, struct proc *reaper,
  ...
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    /*If the process is not the parent, and it is in the same namespace as
     * parent, kill it*/
...
void exit(int status) {
  ...
  *curproc->cwdp = 0;
  ...
  // Here we could not find process with pid 1 inside the namespace, and pid 1
  // wasn't marked as killed
  ...
  // If the current process holds pid 1 within its namespace, mark all child
  // processes as killed
  if (curproc->ns_pid == 1) {
  ...
  if (curproc->child_pid_ns) pid_ns_put(curproc->child_pid_ns);
...
void scheduler(void) {
  ...
  for (;;) {
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      ...
      // Cpu set controller and freezer are only defined on runnable processes
      // which are not killed.
      if (p->killed == 0) {
        // If the cpu set controller enabled, and the cpu doesn't match the one
        // that is supposed to run the process then don't let the process run on
        // this cpu.
        if (p->cgroup->set_controller_enabled &&
            p->cgroup->cpu_to_use != c->apicid) {
          continue;
        }
...
void procdump(void) {
  ...
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    cprintf("%d %s %s", get_pid_for_ns(p, initproc->nsproxy->pid_ns), state,
            p->name);
    if (p->state == SLEEPING) {
...
void proc_lock() { acquire(&ptable.lock); }

void proc_unlock() { release(&ptable.lock); }

/*
 * Move a process from one cgroup to another.
 * Acquires lock, checks if process is alive, calls cgroup_insert, and releases
 * lock.
 */
...
int proc_sys_open(char *path, int omode) {
  ...
  if (*procfs_root == 0) return RESULT_ERROR;

  format_path(fpath, path);
  int i = 1;  // Skip first slash;
  /* Validate if path is procfs dir. */
  for (; i < sizeof(fpath); i++) {
    if (fpath[i] == 0 || fpath[i] == '/') break;
    if (fpath[i] != procfs_dir[i]) return RESULT_ERROR;
  }

  if (i < strlen(procfs_root)) return RESULT_ERROR;
