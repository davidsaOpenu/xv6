...
int handle_bind_mounts() {
  ...
  struct mount *parent = NULL;
  struct vfs_inode *mount_dir = NULL;
  struct vfs_inode *target_mount_dir = NULL;
  int res = -1;
  ...
  mount_dir->i_op->ilock(mount_dir);
  target_mount_dir->i_op->ilock(target_mount_dir);
  ...
  res = mount(mount_dir, NULL, target_mount_dir, parent);

end_locked:
  mount_dir->i_op->iunlock(mount_dir);
  target_mount_dir->i_op->iunlock(target_mount_dir);

end:
  if (mount_dir != NULL) {
    mount_dir->i_op->iput(mount_dir);
  }
  if (target_mount_dir != NULL) {
...
int handle_nativefs_mounts() {
  ...
  char *mount_path = NULL;
  struct mount *parent = NULL;
  struct vfs_inode *loop_inode = NULL, *mount_dir = NULL;
...
end_locked:
  loop_inode->i_op->iunlock(loop_inode);
  mount_dir->i_op->iunlock(mount_dir);
