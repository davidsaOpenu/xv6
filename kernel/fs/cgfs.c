...
static int read_file_io_stat(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(
      stattext + f->off, min(max(stattextp - stattext - f->off, 0), n), addr);
}
...
static int read_file_cg_max_descen(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(
      dec_text + f->off, min(at_least_zero(dec_textp - dec_text - f->off), n),
      addr);
}
...
static int read_file_cg_max_depth(struct vfs_file* f, char* addr, int n) {
  ...
  return copy_buffer_up_to_end(
      depth_text + f->off,
      min(at_least_zero(depth_textp - depth_text - f->off), n), addr);
}
...
int unsafe_cg_write(struct vfs_file* f, char* addr, int n) {
  ...
  if (filename_const == CGROUP_PROCS) {
    ...
    failcnt_string[sizeof(failcnt_string) - 1] = '\0';
