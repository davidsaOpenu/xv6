...
int sys_read(void) {
  struct vfs_file *f;
  int n;
  char *p;  // is it a contiuous space? of what size? should we make it a vector
            // too?
...
static struct vfs_inode *createmount(char *path, short type, short major,
  ...
  if ((ip = dp->sb->ops->ialloc(dp->sb, type)) == 0) panic("create: ialloc");
...
int sys_chdir(void) {
  ...
  if ((cgp = get_cgroup_by_path(path))) {
    end_op();
    safestrcpy(curproc->cwdp, cgp->cgroup_dir_path,
               sizeof(cgp->cgroup_dir_path));
    return 0;
  }
  if ((ip = vfs_nameimount(path, &mnt)) == 0) {
    end_op();
