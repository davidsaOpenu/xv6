...
run_host_test() {
    if [ "$#" -ne 1 ]; then
            echo "ERROR: run_host_test should be used with one argument only!"
            exit 1
    fi

    echo "[*] CI: Running single host test '$1'"
    time ssh -o StrictHostKeyChecking=no -p "$ssh_port" elk@localhost "cd /opt/elk/xv6 && ./xv6/build/tests/host/$1"
    echo "[*] CI: Done running single host test '$1'"
}
...
run_all_guest_tests() {
    ...
    run_test     history_navigation_test
    ...
    run_test     test_pouch_build_valid_args
    run_test     test_pouch_build_invalid_args
    run_test     test_pouch_builds
    run_test     pidns_tests
    run_test     test_internal_images
}

run_all_host_tests() {
    run_host_test buf_cache_tests
    run_host_test kvector_tests
    run_host_test obj_fs_tests
}
...
time copy_to_vm
time build
time run_all_host_tests
time run_all_guest_tests
