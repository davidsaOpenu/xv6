...
static void last_cmd_retval_no_error(void) { last_cmd_retval = 0; }

static void last_cmd_retval_set_error(int err) { last_cmd_retval = err; }
...
// RUN_INTERNAL_CMD_ERROR : There was an error during execution of internal
// command
int runinternal(struct cmd **pcmd) {
