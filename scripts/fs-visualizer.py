...
class XV6FSImage:
    ...
    def _read_all_inodes(self):
        ...
        for i in range(self._sb_ninodes):
            ...
            if (
                i != 0 and inode.type == 0 and not reached_end
            ):  # This is the max allocated inode, sure no inode will be used after this
                reached_end = True
    ...
    def _mark_all_used_blocks(self):
        def add_addr(addr, error_message_note=""):
            if addr in self._used_blocks:
                raise ValueError(
                    f"Block #{addr} is already used ({error_message_note}!"
                )
        ...
        def read_indirect_block(blob: bytes) -> List[int]:
            ...
            while i < BSIZE:
                ...
                if current_addr == 0:
                    ...
                    if has_reached_end:
                        raise ValueError(
                            f"Indirect block has garbage value at offset {i}, value is {current_addr}"
                        )
                    else:
    ...
    def _read_superblock(self):
        ...
        assert (
            self._sb_n_blocks
            == blocks_from_file - 2 - N_LOG_BLOCKS - N_INODE_BLOCKS - 1
        )
        ...
        for idx, byte in enumerate(self._bitmap_block):
            for bi in range(8):
                ...
                if value != value_should_be:
                    print(
                        f"Bitmap bit {current_bit_overall} is {value}, it should be {value_should_be}"
                    )
