...
$(IMG_FS): $(MKFS) $(USER_BINARIES) $(POUCH_BINARY) $(CONTAINER_IMAGES) $(TESTS_GUEST)
	$(MKFS) $@ 0 $(USER_BINARIES) $(CONTAINER_IMAGES) $(TESTS_GUEST)
...
# Pouch Executable
# ------------------------------------------------------------------------------
POUCH_SOURCES := $(wildcard user/pouch/*.[ch])
POUCH_INCLUDES := $(INCLUDE_DIRS_USERLAND) -Iuser

$(POUCH_BINARY): $(POUCH_SOURCES) $(B)/userlib/userlib.a 
	$(CC) $(USER_CFLAGS) $(POUCH_INCLUDES) $(filter-out %.h,$^) -o $@
...
$(CONTAINER_IMAGES):
	...
	$(DOCKER) build -t xv6__$(@F) -f $(filter %.Dockerfile,$^) $(B)/images
	$(DOCKER) save xv6__$(@F) > $(B)/images/$(@F).tar
	mkdir -p $(B)/images/$(@F)
	tar xf $(B)/images/$(@F).tar -C $(B)/images/$(@F)
	$(PYTHON3) ./scripts/make-image.py $(B)/images/$(@F) $(MKFS)
