...
CFLAGS 	:= 	-static -MD -m32 -O2 -mno-sse -std=gnu99 -Wall -Werror \
			-Wstack-usage=4096 -fno-pic -fno-builtin \
			-fno-strict-aliasing -fno-omit-frame-pointer \
		  	-DHOST_CPU_TSC_FREQ=3302*1000 \
			-DXV6_TSC_FREQUENCY=3302*1000 \
...
CONTAINER_IMAGES 	:=	$(addprefix $(B)/images/,$(CONTAINER_IMAGES))
...
TEST_ASSETS			:=  $(wildcard tests/pouchfiles/*.p) $(B)/all_images
...
# Filesystems
# ------------------------------------------------------------------------------
$(IMG_FS): $(MKFS) $(USER_BINARIES) $(POUCH_BINARY) $(CONTAINER_IMAGES) $(TESTS_GUEST) $(TEST_ASSETS)
	$(MKFS) $@ 0 $(USER_BINARIES) $(CONTAINER_IMAGES) $(TESTS_GUEST) $(POUCH_BINARY) $(TEST_ASSETS)
...
$(B)/initcode: kernel/initcode.S
	...
	$(LD) $(LDFLAGS) -N -e start -Ttext 0 $(B)/initcode.o -o $(B)/initcode.out
...
$(B)/entryother: kernel/entryother.S
	...
	$(LD) $(LDFLAGS) -N -e start -Ttext 0x7000 $(B)/entryother.o -o $(B)/bootblockother.o
...
$(B)/bootblock: kernel/bootasm.S kernel/bootmain.c
	...
	$(LD) $(LDFLAGS) -N -e start -Ttext 0x7C00 $(B)/bootasm.o $(B)/bootmain.o  -o $(B)/bootblock.o
...
$(B)/user/%: user/%.c $(B)/userlib/userlib.a
	@echo "\033[35m[USERBUILD] Building $@\033[0m"
	$(CC) $(USER_CFLAGS) $(INCLUDE_DIRS_USERLAND) \
	-c -o $@.o $<
	$(LD) $(LDFLAGS) -T user/userspace.ld -N -e main -Ttext 0 $@.o $(abspath $(B)/userlib/userlib.a) -o $@
...
$(B)/pouch/%.o: user/pouch/%.c
	@echo "\033[36m[POUCH] BUILDING POUCH OBJECT $@\033[0m"
	$(CC) $(USER_CFLAGS) $(POUCH_INCLUDES) -o $@ -c $<  # PouchBuild

$(POUCH_BINARY): $(POUCH_OBJECTS) $(B)/userlib/userlib.a
	@echo "\033[36m[POUCH] LINKING POUCH\033[0m"
	$(LD) $(LDFLAGS) -T user/userspace.ld -N -e main -Ttext 0 $^ -o $@

# Pouch Images
# ------------------------------------------------------------------------------
$(B)/images/internal_fs_a: $(B)/images/user images/img_internal_fs_a.Containerfile images/hello.txt
$(B)/images/internal_fs_b: $(B)/images/user images/img_internal_fs_b.Containerfile images/test.txt
$(B)/images/internal_fs_c: $(B)/images/user images/img_internal_fs_c.Containerfile images/test.txt images/hello.txt

$(CONTAINER_IMAGES):
	@echo ------------------BUILD POUCH IMAGE $(@F)------------------
	mkdir -p $(B)/images/buildroot
	$(PODMAN) build -t xv6__$(@F) -f $(filter %.Containerfile,$^) $(B)/images
	$(PODMAN) save localhost/xv6__$(@F) > $(B)/images/buildroot/$(@F).tar
	mkdir -p $(B)/images/buildroot/$(@F)
	tar -xhf $(B)/images/buildroot/$(@F).tar -C $(B)/images/buildroot/$(@F)
	./scripts/podman-image-extractor.sh $(B)/images/buildroot/$(@F).tar $(B)/images/buildroot $(MKFS)

	# Copy artifact to images for mkfs
	cp -v $(B)/images/buildroot/$(@F)/build/$(@F) $(B)/images/$(@F:img_%=%)
...
# Guest Tests
# ------------------------------------------------------------------------------
GUEST_TEST_CFLAGS := $(CFLAGS) -Itests -Itests/xv6 -I.
GUEST_LDFLAGS := -m elf_i386 -T user/userspace.ld -N -e main -Ttext 0
