...
int add_object(const void* object, uint size, const char* name) {
...
int rewrite_object(const void* object, uint size, const char* name) {
    if (strlen(name) > MAX_OBJECT_NAME_LENGTH) {
        return OBJECT_NAME_TOO_LONG;
    }
    for (uint i = 0; i < max_objects(); ++i) {
...
int object_size(const char* name, uint* output) {
    if (strlen(name) > MAX_OBJECT_NAME_LENGTH) {
        return OBJECT_NAME_TOO_LONG;
    }
    for (uint i = 0; i < max_objects(); ++i) {
...
int get_object(const char* name, void* output) {
    if (strlen(name) > MAX_OBJECT_NAME_LENGTH) {
        return OBJECT_NAME_TOO_LONG;
    }
    for (uint i = 0; i < max_objects(); ++i) {
